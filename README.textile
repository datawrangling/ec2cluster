...
Connect to Elasticwulf REST API
<pre><code>
...
job_id = job.id # save this for later...
</code></pre>

Ping server for status periodically until Job is complete, then fetch output files from S3
<pre><code>
